  ...
  , m_datarate(Crazyradio::Datarate_250KPS)
{
  ...
  char datarateType;
  bool success = false;
  std::cout<<"broadcaster opened!!"<<std::endl;
  success = std::sscanf(link_uri.c_str(), "radio://%d/%d/%d%c/%" SCNx64,
     &m_devId, &channel, &datarate,
     &datarateType, &m_address) == 5;
  if (!success) {
...
static float const ATT_LIMIT = 360.0f; // rad
//static const uint32_t INT24_MAX = 8388607;
static inline attFixed24_t att_float_to_fix24(float x)
{
  uint32_t val = (INT24_MAX / ATT_LIMIT) * (x + ATT_LIMIT);
  attFixed24_t result;
  result.low = (val >> 0) & 0xFF;
  result.middle = (val >> 8) & 0xFF;
  result.high = (val >> 16) & 0xFF;
...
void CrazyflieBroadcaster::sendAttSps(const std::vector<AttSetPts>& data)
{
  if (data.size()==0){
//      std::cout<<"att send---"<<std::endl;
      return;
  }
  std::vector<crtpAttSetPointPacked> requests(ceil(data.size()/4.0));
  for (size_t i = 0; i < data.size(); ++i) {
    size_t j = i / 4;
...
//  for(size_t i=0; i < data.size(); ++i) {
//    size_t j = i/2.0;
//    requests[j].data.attSp[i%2].id = data[i].id;
//    requests[j].data.attSp[i%2].roll = att_float_to_fix24(data[i].roll);
//    requests[j].data.attSp[i%2].pitch = att_float_to_fix24(data[i].pitch);
//    requests[j].data.attSp[i%2].yaw = att_float_to_fix24(data[i].yaw);
//    requests[j].data.attSp[i%2].thrust = att_float_to_fix24(data[i].thrust);
//  }
  ...
  while (remainingRequests > 0) {
    if (remainingRequests >= 2) {
      send2Packets(reinterpret_cast<const uint8_t*>(&requests[i]), 2 * sizeof(crtpAttSetPointPacked));
      remainingRequests -= 2;
      i += 2;
    } else {
      sendPacket(reinterpret_cast<const uint8_t*>(&requests[i]), sizeof(crtpAttSetPointPacked));
      remainingRequests -= 1;
      i += 1;
    }
  }

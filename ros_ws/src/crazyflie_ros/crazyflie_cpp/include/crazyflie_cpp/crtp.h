...
// Port (0x09 Attitude Setpoint)
typedef uint16_t fp16_t;
typedef int16_t posFixed16_t;
typedef struct attFixed24_t
{   uint8_t low;
    uint8_t middle;
    uint8_t high;
} attFixed24_t;
...
struct data_attsp{
    struct{
        ...
        attFixed24_t yaw;
        attFixed24_t thrust;
    }__attribute__((packed)) attSp[2];
}__attribute__((packed));

struct crtpCommanderAttSetPointRequest
{
    crtpCommanderAttSetPointRequest()
        : header(0x09, 0)
        {
            data.attSp[0].id = 0;
            data.attSp[1].id = 0;
    ...
    const crtp header;
    struct data_attsp data;
}__attribute__((packed));
CHECKSIZE(crtpCommanderAttSetPointRequest)

struct crtpAttSetPointPacked
{
    crtpAttSetPointPacked()
            : header(0x06, 3) //09 2
    {
    }
    const crtp header;
...
CHECKSIZE(crtpAttSetPointPacked)
